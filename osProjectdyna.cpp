...
void input()																			//to take the input about the process
{
	...
	for( i=0 ; i<n ; i++)
	{
		...
		p[i].pNo=i+1;
		printf("\t\t\t\tPROCESS P%d : \n",i+1);
	a:  printf("\t\t\t\t\tArrival  time : ",i+1);
		scanf("%d",&p[i].at);
		if(p[i].at<0)
		{
			printf("No of arrival time should be more than 0 \n");
			goto a;
		}
	b:	printf("\t\t\t\t\tBurst    time : ",i+1);
		scanf("%d",&p[i].bt);
		if(p[i].bt<0)
		{
			printf("No of burst time should be more than 0 \n");
			goto b;
		}
		p[i].life = true;
		system("@cls||clear");
...
{
	char c;
 y: printf("\t\t\t\tENTER NO OF PROCESS : ");
	scanf("%d",&n);
 	if(n<1)
	{
		printf("No of process should be more than 0\n");
		getch();
		goto y;
	}
